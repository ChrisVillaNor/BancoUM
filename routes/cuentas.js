...
router.post('/', async (req, res) => {
  const { usuario_id, tipo_cuenta_id, numero_cuenta, saldo, cupo_total } = req.body;
  try {
    const query = `
      INSERT INTO cuenta (
        usuario_id, tipo_cuenta_id, numero_cuenta, saldo, 
        cupo_total, cupo_disponible, con_cuota_manejo, cuota_manejo_valor, 
        tasa_interes, fecha_apertura, activa
      ) 
      VALUES ($1, $2, $3, $4, $5, $6, false, 0, 0, NOW(), true)
      RETURNING *;
    `;
    const saldoNum = parseFloat(saldo) || 0;
    const cupoNum = parseFloat(cupo_total) || 0;

    const values = [usuario_id || 1, tipo_cuenta_id || 1, numero_cuenta, saldoNum, cupoNum, cupoNum];
